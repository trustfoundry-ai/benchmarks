...
import {
  trustfoundryCitationLookupScorerAdapter,
  _internals,
  SUPPORTED_CUTOFFS,
  SUPPORTED_HEADLINE_CUTOFF
} from '../../../src/adapters/scorers/trustfoundry-citation-lookup.mjs';
...
function positiveCase({
  ...
  return {
    caseId,
    benchmarkId: 'trustfoundry-citation-lookup',
...
async function scoreOne(benchmarkCase, env, resultOverrides = {}) {
  ...
  return trustfoundryCitationLookupScorerAdapter.score({
    manifest: null,
    cases: [benchmarkCase],
    providerResults
  });
...
test('negatives excluded from positives denominator; fp_rate reflects negatives-only', async () => {
  ...
  const scored = await trustfoundryCitationLookupScorerAdapter.score({
    manifest: null,
...
test('filter guard: only cases with benchmarkId=trustfoundry-citation-lookup are scored', async () => {
...
test('scoreStream matches score() shape and skips foreign benchmarks', async () => {
  ...
  const scored = await trustfoundryCitationLookupScorerAdapter.scoreStream({
    manifest: null,
    pairs: asIterable()
  });
  assert.equal(scored.caseScores.length, 1);
...
test('exports SUPPORTED_CUTOFFS + SUPPORTED_HEADLINE_CUTOFF for the runner validator', () => {
  ...
  assert.deepEqual(trustfoundryCitationLookupScorerAdapter.SUPPORTED_CUTOFFS, [1, 5, 10, 25]);
  assert.equal(trustfoundryCitationLookupScorerAdapter.SUPPORTED_HEADLINE_CUTOFF, 1);
});
