...
test('loads a single dataset with correct case/metadata shape', async () => {
  await withTempDir(async (root) => {
    const dsDir = path.join(root, 'data', 'citation-lookup-cases');
    await mkdir(dsDir, { recursive: true });
    const file = path.join(dsDir, 'dataset.jsonl');
    await writeJsonl(file, [positiveCaseRow()]);

    const { cases, benchmark, inventory } = await citationLookupBenchmarkAdapter.loadCases({
      config: { datasetPath: 'data/citation-lookup-cases/dataset.jsonl' },
      repoRoot: root
    });
    ...
    assert.equal(only.benchmarkId, 'citation-lookup');
...
test('geo normalization: non-empty geo_level_2 uppercases; empty becomes FED', async () => {
  await withTempDir(async (root) => {
    const dsDir = path.join(root, 'data', 'citation-lookup-statutes');
    await mkdir(dsDir, { recursive: true });
    await writeJsonl(path.join(dsDir, 'dataset.jsonl'), [statuteRow(), positiveCaseRow()]);

    const { cases } = await citationLookupBenchmarkAdapter.loadCases({
      config: { datasetPath: 'data/citation-lookup-statutes/dataset.jsonl' },
      repoRoot: root
    });
...
test('negatives propagate kind and negative_category', async () => {
  await withTempDir(async (root) => {
    const dsDir = path.join(root, 'data', 'citation-lookup-negatives');
    await mkdir(dsDir, { recursive: true });
    await writeJsonl(path.join(dsDir, 'dataset.jsonl'), [negativeRow()]);

    const { cases } = await citationLookupBenchmarkAdapter.loadCases({
      config: { datasetPath: 'data/citation-lookup-negatives/dataset.jsonl' },
      repoRoot: root
    });
...
test('datasetPaths combined config stitches rows and tags per-dataset name', async () => {
  await withTempDir(async (root) => {
    const casesDir = path.join(root, 'data', 'citation-lookup-cases');
    const statutesDir = path.join(root, 'data', 'citation-lookup-statutes');
    const regsDir = path.join(root, 'data', 'citation-lookup-regulations');
    const negsDir = path.join(root, 'data', 'citation-lookup-negatives');
    ...
    const { cases, inventory } = await citationLookupBenchmarkAdapter.loadCases({
      config: {
        datasetPaths: [
          'data/citation-lookup-cases/dataset.jsonl',
          'data/citation-lookup-statutes/dataset.jsonl',
          'data/citation-lookup-regulations/dataset.jsonl',
          'data/citation-lookup-negatives/dataset.jsonl'
        ]
      },
      repoRoot: root
    });
    ...
    assert.deepEqual(
      ...
      [
        'citation-lookup-cases',
        'citation-lookup-statutes',
        'citation-lookup-regulations',
        'citation-lookup-negatives'
      ]
    );
...
test('limit config caps case count', async () => {
  await withTempDir(async (root) => {
    ...
    const { cases, inventory } = await citationLookupBenchmarkAdapter.loadCases({
      config: { datasetPath: 'data/citation-lookup-cases/dataset.jsonl', limit: 2 },
      repoRoot: root
    });
...
test('missing dataset file throws a clear error', async () => {
  await withTempDir(async (root) => {
    await assert.rejects(
      citationLookupBenchmarkAdapter.loadCases({
        config: { datasetPath: 'data/citation-lookup-cases/dataset.jsonl' },
        repoRoot: root
      }),
      /citation-lookup dataset not found/
    );
  });
});
...
test('config missing both datasetPath and datasetPaths throws', async () => {
  await assert.rejects(
    citationLookupBenchmarkAdapter.loadCases({ config: {}, repoRoot: '/tmp' }),
    /datasetPath/
  );
});
