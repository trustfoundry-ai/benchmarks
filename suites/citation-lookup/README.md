...
The runner reads the scorer id from the benchmark config (`"scorer": "citation-lookup"`), so `--scorer-config` above is optional as long as the config path in the default location matches the scorer's cutoffs.
...
2. `cutoffs` and `headline_cutoff` match the values the scorer implementation is actually computing. If you need different K values, update `src/adapters/scorers/citation-lookup.mjs` and the artifact schema together.
