...
const MODEL_TYPES = {
  case_questions: 'case_question',
  case_key_facts: 'case_key_fact',
  laws: 'law_question',
  regs: 'reg_question'
};
...
function stateForRow(row) {
  ...
  const datasource = row.datasource_id ?? '';
  if (datasource === 'us-usc' || datasource === 'us-ecfr') return 'FED';
  const match = /^([a-z]{2})-(?:laws|regs)$/.exec(datasource);
...
function buildCase(row, { datasetName, index, datasetSize, queryTransformId }) {
  ...
  const modelType = row.model_type ?? MODEL_TYPES[datasetName] ?? null;
  const caseId =
    row.caseId ??
    row.case_id ??
    `trustfoundry-legal-search:${datasetName}:${split}:${index}`;
...
function includeCase(benchmarkCase, filters) {
  if (filters.docTypes && !filters.docTypes.includes(benchmarkCase.metadata.doc_type)) {
    return false;
  }
  if (filters.fields && !filters.fields.includes(benchmarkCase.metadata.field)) {
    return false;
  }
...
export const trustfoundryLegalSearchBenchmarkAdapter = {
  id: 'trustfoundry-legal-search',
  version: 'trustfoundry-legal-search-v1',
  materializationVersion: 'trustfoundry-legal-search-data-v1',
  ...
  async loadCases({ config, repoRoot }) {
    ...
    const filters = {
      docTypes: normalizeList(config.docTypes),
      fields: normalizeList(config.fields),
