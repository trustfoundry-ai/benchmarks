...
const VERSION = 'trustfoundry-citation-lookup-v1';
...
function datasetNameFor(filePath) {
  ...
  if (!parent || parent === '.') return fileStem(filePath);
  return parent;
}
...
async function resolveDatasetPaths(config, repoRoot) {
  ...
  if (!configured) {
    throw new Error(
      "trustfoundry-citation-lookup: config must specify 'datasetPath' or 'datasetPaths'"
    );
  }
  ...
  for (const p of configured) {
    ...
    const stats = await stat(abs).catch((error) => {
      throw new Error(
        `trustfoundry-citation-lookup dataset not found at ${abs}. (${error.message})`
      );
    });
    if (!stats.isFile()) {
      throw new Error(`trustfoundry-citation-lookup dataset path is not a regular file: ${abs}`);
    }
    resolved.push(abs);
  }
  return resolved;
...
function buildCase(row, { filePath, index }) {
  ...
  return {
    ...
    scoringHints: { kind: 'trustfoundry-citation-lookup', outputMode: 'json' }
  };
}

export const trustfoundryCitationLookupBenchmarkAdapter = {
  id: BENCHMARK_ID,
  version: VERSION,
  promptVersion: 'trustfoundry-citation-lookup-prompt-v1',
  materializationVersion: 'trustfoundry-citation-lookup-data-v1',
